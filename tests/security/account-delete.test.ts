 ...
 * cause: DELETE /api/account wiped app data but never deleted the Clerk user,
...
const deleteUser = vi.fn<(id: string) => Promise<unknown>>(async () => ({}));
...
beforeEach(() => {
  ...
  vi.mocked(clerkConfigured).mockReturnValue(true);
});
...
describe("account deletion fully frees the email for re-signup", () => {
  ...
  it("demo and guest identities never reach the auth provider", async () => {
    ...
    vi.mocked(clerkConfigured).mockReturnValue(false);
    const res = await DELETE(deleteReq());
    expect(res.status).toBe(200);
