...
for (const vp of VIEWPORTS) {
  test.describe(`${vp.name} (${vp.width}px)`, () => {
    ...
    test("no uncaught errors + no overflow across surfaces", async ({ page }) => {
      ...
      for (const path of ["/", "/product", "/operators", "/demo", "/templates", "/security", "/pricing", "/privacy", "/terms", "/app", "/app/missions", "/app/decisions", "/app/automations", "/app/memory", "/app/activity", "/app/account", "/sign-in"]) {
