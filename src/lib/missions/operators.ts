...
export const OPERATOR_PROFILES: Record<string, OperatorProfile> = {
  ...
  computer: {
    ...
    responsibility: "looks at the screen and, once approved, makes the inputs a step named.",
    tools: ["computer.observe", "computer.operate"],
    maxRuntimeMs: 60_000,
    maxRetries: 1,
    never: "clicks or types anything without an approved card listing every input.",
  },
  research: {
    key: "research",
    name: "Research Operator",
    responsibility: "compares evidence and produces structured findings with sources.",
    tools: ["analyze.extract", "analyze.compare", "compute.run", "inbox.summarize", "laptop.compare", "laptop.recommend"],
    maxRuntimeMs: 45_000,
    maxRetries: 1,
