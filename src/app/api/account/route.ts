...
import { clerkConfigured, DEMO_USER_ID } from "@/lib/auth";
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (clerkConfigured() && userId !== DEMO_USER_ID && !isGuestId(userId)) {
      try {
        const { clerkClient } = await import("@clerk/nextjs/server");
        const client = await clerkClient();
        await client.users.deleteUser(userId);
      } catch (err) {
        // Already gone (a retry) is success; anything else is surfaced
        // honestly — the data is wiped, and this call is safe to retry.
