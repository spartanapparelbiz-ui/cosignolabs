-- cosigno — database setup (SAFE / re-runnable version).
-- Paste into Supabase → SQL Editor → New query → Run.
-- This version SKIPS anything that already exists, so it's safe to run even if
-- your database was partly set up before. Expect "Success. No rows returned."
--
-- ============================================================
-- 0001_init.sql
-- ============================================================
...
do $$ begin
  create type action_status as enum ('proposed', 'approved', 'vetoed', 'executing', 'executed', 'failed');
exception when duplicate_object then null;
end $$;

do $$ begin
  create type action_event_type as enum ('proposed', 'approved', 'edited', 'vetoed', 'executing', 'executed',
...
create index if not exists actions_status_idx on actions (user_id, status);
...
create index if not exists action_events_user_idx on action_events (user_id, created_at desc);
...
drop trigger if exists actions_transition_guard on actions;
create trigger actions_transition_guard before update on actions
...
drop policy if exists account_audit_self on account_audit;
...
drop policy if exists promotions_self on promotions;
...
create index if not exists connections_user_idx on connections (user_id, created_at desc);
...
create index if not exists mcp_tools_user_idx on mcp_tools (user_id);
...
drop policy if exists mcp_tools_self on mcp_tools;
...
create index if not exists automations_user_idx on automations (user_id, created_at desc);
-- the tick's hot path: enabled + due
create index if not exists automations_due_idx on automations (next_run_at) where enabled;
...
drop policy if exists automations_self on automations;
...
drop policy if exists automation_runs_self on automation_runs;
...
create index if not exists files_user_idx on files (user_id, updated_at desc);
...
drop policy if exists files_self on files;
...
create index if not exists missions_runnable_idx on missions (state) where state in ('queued','running','retrying','verifying');
...
create index if not exists mission_steps_mission_idx on mission_steps (mission_id, idx);
create index if not exists mission_steps_user_idx on mission_steps (user_id);
...
drop policy if exists mission_steps_self on mission_steps;
...
alter table missions add column if not exists lease_owner text;
alter table missions add column if not exists lease_expires_at timestamptz;
-- Cost/resource controls, counted as work happens.
alter table missions add column if not exists tool_calls int not null default 0;
alter table missions add column if not exists browser_actions int not null default 0;
alter table missions add column if not exists budget_cents int not null default 200;
...
create index if not exists browser_sessions_mission_idx on browser_sessions (mission_id);
...
create index if not exists browser_actions_session_idx on browser_actions (session_id, idx);
create index if not exists browser_actions_user_idx on browser_actions (user_id);
...
drop policy if exists browser_sessions_self on browser_sessions;
...
drop policy if exists browser_actions_self on browser_actions;
...
create index if not exists mission_sources_user_idx on mission_sources (user_id, created_at desc);
create index if not exists mission_sources_mission_idx on mission_sources (mission_id);
-- staged (unattached) sources for the ask box
create index if not exists mission_sources_staged_idx on mission_sources (user_id) where mission_id is null;
...
drop policy if exists mission_sources_self on mission_sources;
...
alter table browser_sessions add column if not exists screenshot_ref text;

create table if not exists browser_products (
  id uuid primary key default gen_random_uuid(),
  user_id text not null,
  mission_id uuid not null references missions (id) on delete cascade,
  session_id uuid not null references browser_sessions (id) on delete cascade,
...
create index if not exists browser_products_mission_idx on browser_products (mission_id, created_at);
create index if not exists browser_products_user_idx on browser_products (user_id);
...
drop policy if exists browser_products_self on browser_products;
...
create table if not exists autopilot_signal_states (
  user_id text not null,
...
drop policy if exists autopilot_signal_states_self on autopilot_signal_states;
...
drop policy if exists autopilot_meta_self on autopilot_meta;
...
create table if not exists temporary_authority (
  id uuid primary key default gen_random_uuid(),
  user_id text not null,
  category text not null,
...
create index if not exists temporary_authority_user_idx on temporary_authority (user_id, created_at desc);
-- the resolver's hot path: live grants for a user
create index if not exists temporary_authority_live_idx on temporary_authority (user_id, expires_at)
...
drop policy if exists temporary_authority_self on temporary_authority;
...
create index if not exists objectives_user_idx on objectives (user_id, created_at desc);
...
create index if not exists objective_links_user_idx on objective_links (user_id);
create index if not exists objective_links_session_idx on objective_links (session_id);

create table if not exists user_hold (
  user_id text primary key,
...
drop policy if exists objectives_self on objectives;
...
drop policy if exists objective_links_self on objective_links;
...
drop policy if exists user_hold_self on user_hold;
...
create index if not exists permission_rules_user_idx on permission_rules (user_id, created_at desc);
...
drop policy if exists permission_rules_self on permission_rules;
...
create index if not exists ai_usage_mission_idx on ai_usage (mission_id) where mission_id is not null;
